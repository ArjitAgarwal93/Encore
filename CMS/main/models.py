...
class Vote(models.Model):   #add this class and the following fields
	...
	def __str__(self):
		return f'{self.profile.user} voted for the {self.product}.'

	# @staticmethod
	# def update_product_averages(product):
	#     Vote.objects.filter(product=product).aggregate(
	#         comfort_avg=Avg('comfort'),
	#         performance_avg=Avg('performance'),
	#         durability_avg=Avg('durability')
	#     )
	#     product.save()
